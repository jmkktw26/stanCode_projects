...
python3 babynames.py -search aa data/full/baby-2000.txt data/full/baby-2010.txt
    ...
    with open(filename, "r")as txt:
                ...
                rank_name_data = txt.readline()
                while rank_name_data != "":
                    data = rank_name_data.split(",")
                    rank = data[0].strip()
                    name1 = data[1].strip()
                    add_data_for_name(name_data, year, rank, name1)
                    name2 = data[2].strip()
                    add_data_for_name(name_data, year, rank, name2)
                    rank_name_data = txt.readline().strip()
    txt.close()
...
def search_names(name_data, target):
    ...
    """
    names = [names for names in name_data.keys() if target.lower() in names.lower()]
    return names
